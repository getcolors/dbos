(ns io.github.getcolors.dbos.ssh-config-test
...
(defn- with-config
  ...
  [content f]
  (let [home (str (fs/create-temp-dir {:prefix "dbos-ssh-config"}))
...
(deftest alias-is-the-profile
  (is (= "dbos-fixture" (ssh-config/host-alias (fixture)))))
...
(deftest identity-file-keeps-the-tilde
  ...
  (is (= "~/.ssh/dbos-fixture" (ssh-config/identity-file (fixture))))
...
(deftest the-marker-is-the-alias-alone
  ...
  (is (= "# BEGIN dbos-digitalocean ANSIBLE MANAGED BLOCK"
         (ssh-config/begin-marker "dbos-digitalocean")))
  (is (= "# END dbos-digitalocean ANSIBLE MANAGED BLOCK"
         (ssh-config/end-marker "dbos-digitalocean"))))
...
(deftest owned-markers-hold-the-one-marker
  ...
  (is (= {:begin #{"# BEGIN dbos-digitalocean ANSIBLE MANAGED BLOCK"}
          :end #{"# END dbos-digitalocean ANSIBLE MANAGED BLOCK"}}
         (ssh-config/owned-markers "dbos-digitalocean"))))

;; §5 never adopt

(deftest host-patterns-are-read-from-a-host-line
  (is (= ["dbos-fixture"] (ssh-config/host-patterns "Host dbos-fixture")))
  (is (= ["web" "dbos-fixture" "db"] (ssh-config/host-patterns "  host   web dbos-fixture  db ")))
  (is (nil? (ssh-config/host-patterns "    HostName 192.0.2.1")))
  (is (nil? (ssh-config/host-patterns "Match host dbos-fixture"))))

(deftest a-foreign-stanza-is-found
  (let [lines ["Host other" "    HostName 192.0.2.1" "" "Host dbos-fixture"]]
    (is (= 4 (ssh-config/foreign-stanza-line lines "dbos-fixture")))))

(deftest our-own-block-is-not-foreign
  (let [alias "dbos-fixture"
...
(deftest a-block-under-a-package-prefixed-marker-is-foreign
  ...
  (let [alias "dbos-digitalocean"
        lines [(str "# BEGIN dbos " alias " ANSIBLE MANAGED BLOCK")
               (str "Host " alias)
               (str "# END dbos " alias " ANSIBLE MANAGED BLOCK")]]
    (is (= 2 (ssh-config/foreign-stanza-line lines alias)))))

(deftest a-multi-pattern-host-line-counts
  (is (= 1 (ssh-config/foreign-stanza-line ["Host web dbos-fixture db"]
                                           "dbos-fixture"))))

(deftest an-unrelated-file-is-left-alone
  (is (nil? (ssh-config/foreign-stanza-line ["Host build" "Host dbos-other"]
                                            "dbos-fixture"))))
  ...
  (with-config "Host other\n    HostName 192.0.2.1\n\nHost dbos-fixture\n    User root\n"
    (fn [file]
      (let [err (ssh-config/adopt-error (fixture))]
        ...
        (is (str/includes? err "`Host dbos-fixture` at line 4"))
        (is (str/includes? err "will not overwrite it"))))))

(deftest adopt-error-passes-our-own-block-and-a-missing-file
  (with-config (str (ssh-config/begin-marker "dbos-fixture") "\n"
                    "Host dbos-fixture\n    HostName 192.0.2.1\n"
                    (ssh-config/end-marker "dbos-fixture") "\n")
...
(deftest the-local-play-renders-no-address
  ...
  (let [data (tools/ansible-local-data (assoc (fixture) :ip "203.0.113.7"))]
    ...
    (is (= "~/.ssh/dbos-fixture" (:ssh-config-identity-file data)))))
...
(deftest the-local-stage-renders-three-files
  (let [targets (map #(str (:target %)) (tools/ansible-local-specs (fixture)))]
    ...
    (is (every? #(str/includes? % "dbos-ansible-local") targets))))
