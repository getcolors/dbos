...
(defn stamp-green [s sha]
  (when (re-find #"\(def \^:private dbos-sha (?:nil|\"[0-9a-f]{40}\")\)" s)
    (str/replace-first s #"\(def \^:private dbos-sha (?:nil|\"[0-9a-f]{40}\")\)"
                       (str "(def ^:private dbos-sha \"" sha "\")"))))

(defn stamp-red [s sha]
  (let [pinned (str "\"package-dbos-red\": \"github:getcolors/dbos#" sha "\",")]
    (cond (str/includes? s "\"package-dbos-red\": null,")
          (str/replace-first s "\"package-dbos-red\": null," pinned)
          (re-find #"\"package-dbos-red\": \"github:getcolors/dbos#[0-9a-f]{40}\"," s)
          (str/replace-first s #"\"package-dbos-red\": \"github:getcolors/dbos#[0-9a-f]{40}\"," pinned))))
...
(defn blue-pinned-meta [sha]
  (str "# dependencies = [\"package-dbos-blue\", \"blue\", \"package-once-blue\"]\n"
       ...
       "# package-once-blue = { git = \"https://github.com/getcolors/once.git\", rev = \"69527114b8bd1ead0b92dc1b08e6bf9a446c341a\", subdirectory = \"blue\" }\n"
       "#\n"
       ;; package-once-blue at the frozen ONCE pin carries its own, older blue
       ;; pin (369c5aa); the override makes this package's blue pin win, as it
       ;; does in blue/pyproject.toml.
       "# [tool.uv]\n"
...
(defn stamp-blue [s sha]
  ...
  (cond (str/includes? s blue-unpinned-meta)
        (-> s
            ...
            (str/replace-first #"(?s)# UNPINNED:.*?DBOS_LIB_ROOT=/path/to/dbos\n"
                               "# Stamped by `bb pin`. DBOS_LIB_ROOT=/path/to/dbos still overrides the\n# pin with a working tree.\n"))
        (re-find #"dbos\.git\", rev = \"[0-9a-f]{40}\"" s)
        (str/replace-first s #"dbos\.git\", rev = \"[0-9a-f]{40}\""
                           (str "dbos.git\", rev = \"" sha "\""))))

(def sites
  [{:path "../skills/package-dbos-green/green" :stamp stamp-green}
   {:path "../skills/package-dbos-red/red" :stamp stamp-red}
   {:path "../skills/package-dbos-blue/blue" :stamp stamp-blue}])

(let [dirty (git "status" "--porcelain") sha (git "rev-parse" "HEAD") remotes (git "branch" "-r" "--contains" sha)]
  (cond (seq dirty) (do (binding [*out* *err*] (println "dbos working tree is dirty; commit before pinning")) (System/exit 2))
        (not (str/includes? (str remotes) "origin/")) (do (binding [*out* *err*] (println "dbos HEAD is not pushed")) (System/exit 2))
