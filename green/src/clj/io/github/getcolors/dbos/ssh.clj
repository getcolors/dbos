(ns io.github.getcolors.dbos.ssh
  ...
  own token), and a cleanup that runs only after a successful destroy. Reusing
  ...
  from `$HOME` and does not commit rendered output; dbos does commit goldens,
...
(defn with-machine-key
  ...
  (if-not (validate/keygen? opts)
    ...
    (let [build? (rendered-only? opts)
      ...
      (if-not build?
        opts
        (let [profile (or (:profile opts) "dbos")
