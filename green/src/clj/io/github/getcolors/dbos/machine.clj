(ns io.github.getcolors.dbos.machine
...
(defn clean [opts] (apply dissoc opts [:digitalocean-ssh-key-name :digitalocean-ssh-private-key :digitalocean-ssh-authorized-keys :digitalocean-https-sources :digitalocean-vpc-mode]))
...
(defn requirements [opts]
  {:single_host true :private false
   :security {:egress "all" :private_filter false
              :ingress (vec (keep (fn [[id port]]
                               (let [suffix (if (= id "ssh") "ssh-sources" "http-sources")
                                     sources (request/source-cidrs opts suffix (str "compute-" suffix))]
                                 (when (and (= id "ssh") (not (seq sources))) (throw (ex-info (str "compute-" suffix " is required") {})))
                                 (when (seq sources) {:id id :protocol "tcp" :from_port port :to_port port :sources sources})))
                             [["ssh" 22] ["http" 80] ["https" 443]]))}
   :legacy_state_keys [(str (:profile opts) "/tofu-compute.tfstate")]})
(defn errors [opts]
  (let [errors (compute/validate (clean opts))]
    (if (seq errors) errors
        (try (planning/plan-deployment (clean opts) topology (requirements opts)) []
...
(defn params [opts result]
  (let [node (first (get-in result [:cluster :nodes]))
    ...
    (cond-> (assoc node :ssh-keygen (= "managed" (:mode (ssh/mode (clean opts)))))
...
(defn fallback-params [opts]
  ...
  (params opts (planning/plan-deployment (clean opts) topology (requirements opts))))
...
(defn step [opts]
  (let [planning? (or (= :build (:green/event opts)) (:green/dry-run opts))
        result (if planning? (planning/plan-deployment (clean opts) topology (requirements opts))
                   (orchestration/orchestrate (clean opts) topology (requirements opts)))]
    (when planning?
...
(defn load-inventory
  ...
  ([opts env]
   (let [result (inspection/read-deployment (clean opts) env)]
