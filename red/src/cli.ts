...
import { dbosWorkflow } from "./workflow.ts";

// DBOS and PostgreSQL backups are always-on services, so this package
// deliberately has no stop/start power verbs.
...
export const usage =
  ...
  "  create   provision ONCE and deploy DBOS with PostgreSQL\n" +
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(dbosWorkflow, withFile);
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(dbosWorkflow, defaultArgs(args));
  }
