export const clean = (opts:Opts):Opts => Object.fromEntries(Object.entries(opts).filter(([key])=>!["digitalocean-ssh-key-name", "digitalocean-ssh-private-key", "digitalocean-ssh-authorized-keys", "digitalocean-https-sources", "digitalocean-vpc-mode"].includes(key)));
/** DBOS's single host requirement and application parameter adapter. */
...
export function errors(opts: Opts): string[] {
  const result = validate(clean(opts));
  if (!result.length) { try { plan_deployment(clean(opts),topology,requirements(opts)); } catch (error) { result.push((error as Error).message); } }
  return result;
}
export function params(opts: Opts, result: any): Opts {
  ...
  return {...node,'ssh-keygen':keyMode(clean(opts)).mode==='managed',...(path?{'ssh-private-key-path':path}:{})};
}
export function fallbackParams(opts: Opts): Opts {
  ...
  return params(opts,plan_deployment(clean(opts),topology,requirements(opts)));
}
...
export async function step(opts: Opts): Promise<Opts> {
  ...
  const result:any=planning?plan_deployment(clean(opts),topology,requirements(opts)):await orchestrate(clean(opts),topology,requirements(opts));
...
export async function load(opts:Opts, env:Record<string,string|undefined>=process.env):Promise<Opts>{
  const result:any=await read_deployment(clean(opts),env);
  if(result.status==='destroyed' && opts['red/event']==='delete')return {...opts,'colors-compute/already-destroyed':true,'red/exit':0};
