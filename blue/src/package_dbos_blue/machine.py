...
def clean(opts):
    return {k:v for k,v in opts.items() if k not in ['digitalocean-ssh-key-name', 'digitalocean-ssh-private-key', 'digitalocean-ssh-authorized-keys', 'digitalocean-https-sources', 'digitalocean-vpc-mode']}
...
def errors(opts):
    result = validate(clean(opts))
    if not result:
        try:
            plan_deployment(clean(opts), TOPOLOGY, requirements(opts))
...
def params(opts, result):
    ...
    managed = _mode(clean(opts))['mode'] == 'managed'
...
async def step(opts):
    ...
    result = plan_deployment(clean(opts), TOPOLOGY, requirements(opts)) if planning else await orchestrate(clean(opts), TOPOLOGY, requirements(opts))
...
async def load(opts, env=None):
    result = await read_deployment(clean(opts), env)
