...
from package_dbos_blue import ssh
...
def test_real_events_render_the_real_path(home):
    ...
    assert opts["ssh-private-key-path"] == str(home / ".ssh" / "dbos-keygen-fixture")
    assert opts["ssh-public-key-path"] == str(home / ".ssh" / "dbos-keygen-fixture.pub")
...
def test_opt_out_passes_through_untouched(home):
    for event in ["build", "create", "delete"]:
        ...
        assert opts["digitalocean-ssh-keys"] == "00000000"
...
async def test_first_create_generates_the_keypair(home):
    ...
    prv = home / ".ssh" / "dbos-keygen-fixture"
    pub = home / ".ssh" / "dbos-keygen-fixture.pub"
    ...
    assert "dbos-keygen-fixture managed by Colors" in pub.read_text()
...
async def test_a_key_without_state_is_never_overwritten(home):
    ...
    write(home / ".ssh" / "dbos-keygen-fixture.pub", "ssh-ed25519 AAAA test")
    opts = await ssh.ensure_key({**keygen(), "blue/event": "create"}, none_state)
    ...
    assert prv.read_text() == "irreplaceable"


async def test_state_without_a_key_is_an_error(home):
    opts = await ssh.ensure_key({**keygen(), "blue/event": "create"}, state({"ip": "192.0.2.10"}))
    assert opts["blue/exit"] == 1
...
async def test_opt_out_generates_nothing(home):
    ...
    assert not (home / ".ssh").exists(), "opt-out mode must not touch ~/.ssh"
...
def test_preflight_refuses_a_foreign_key_and_says_do_not_delete_it(home):
    write(home / ".ssh" / "dbos-keygen-fixture.pub", "ssh-ed25519 OURS comment")
    opts = ssh.preflight(
        ssh.with_machine_key({**keygen(), "blue/event": "create"}),
        lambda _p, _t: [{"id": "abc", "name": "dbos-keygen-fixture", "public": "ssh-ed25519 THEIRS"}])
    assert opts["blue/exit"] == 1
...
def test_delete_removes_the_keypair(home):
    write(home / ".ssh" / "dbos-keygen-fixture", "private")
    write(home / ".ssh" / "dbos-keygen-fixture.pub", "public")
    ssh.cleanup_step({**keygen(), "blue/event": "delete", "ssh-keygen": True})
    assert not (home / ".ssh" / "dbos-keygen-fixture").exists()
    assert not (home / ".ssh" / "dbos-keygen-fixture.pub").exists()
...
def test_cleanup_is_inert_on_create_and_in_opt_out_mode(home):
    ...
    assert (home / ".ssh" / "dbos-keygen-fixture").exists()
