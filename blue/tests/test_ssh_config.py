...
from package_dbos_blue import ssh_config, tools, workflow
...
def test_alias_is_the_profile():
    assert ssh_config.host_alias(fixture()) == "dbos-fixture"
...
def test_identity_file_keeps_the_tilde(home):
    ...
    assert ssh_config.identity_file(fixture()) == "~/.ssh/dbos-fixture"
...
def test_the_marker_is_the_alias_alone():
    ...
    # too would repeat it: "# BEGIN dbos dbos-digitalocean".
    assert ssh_config.begin_marker("dbos-digitalocean") == "# BEGIN dbos-digitalocean ANSIBLE MANAGED BLOCK"
    assert ssh_config.end_marker("dbos-digitalocean") == "# END dbos-digitalocean ANSIBLE MANAGED BLOCK"
...
def test_owned_markers_hold_the_one_marker():
    ...
    assert ssh_config.owned_markers("dbos-digitalocean") == {
        "begin": {"# BEGIN dbos-digitalocean ANSIBLE MANAGED BLOCK"},
        "end": {"# END dbos-digitalocean ANSIBLE MANAGED BLOCK"}}
...
def test_host_patterns_are_read_from_a_host_line():
    assert ssh_config.host_patterns("Host dbos-fixture") == ["dbos-fixture"]
    assert ssh_config.host_patterns("  host   web dbos-fixture  db ") == ["web", "dbos-fixture", "db"]
    assert ssh_config.host_patterns("    HostName 192.0.2.1") is None
    assert ssh_config.host_patterns("Match host dbos-fixture") is None


def test_a_foreign_stanza_is_found():
    lines = ["Host other", "    HostName 192.0.2.1", "", "Host dbos-fixture"]
    assert ssh_config.foreign_stanza_line(lines, "dbos-fixture") == 4
...
def test_a_block_under_a_package_prefixed_marker_is_foreign():
    # This package never wrote a `# BEGIN dbos <alias>` marker, so a block
    # carrying one belongs to nobody this package knows and must stop the run
    # rather than being silently overwritten.
    alias = "dbos-digitalocean"
    lines = [f"# BEGIN dbos {alias} ANSIBLE MANAGED BLOCK",
             f"Host {alias}",
             f"# END dbos {alias} ANSIBLE MANAGED BLOCK"]
    assert ssh_config.foreign_stanza_line(lines, alias) == 2


def test_a_multi_pattern_host_line_counts():
    assert ssh_config.foreign_stanza_line(["Host web dbos-fixture db"], "dbos-fixture") == 1


def test_an_unrelated_file_is_left_alone():
    assert ssh_config.foreign_stanza_line(["Host build", "Host dbos-other"],
                                          "dbos-fixture") is None
...
def test_adopt_error_names_the_file_and_the_line(home):
    ...
    assert "`Host dbos-fixture` at line 4" in error
...
def test_adopt_error_passes_our_own_block_and_a_missing_file(home):
    ...
    write_config(home, f"{ssh_config.begin_marker('dbos-fixture')}\n"
                       "Host dbos-fixture\n    HostName 192.0.2.1\n"
                       f"{ssh_config.end_marker('dbos-fixture')}\n")
    assert ssh_config.adopt_error(fixture()) is None
...
def test_preflight_reads_the_redirected_file(home):
    ...
    write_config(home, "Host dbos-fixture\n    HostName 192.0.2.1\n")
...
def test_the_local_play_renders_no_address():
    ...
    assert data["ssh-config-identity-file"] == "~/.ssh/dbos-fixture"
...
def test_the_local_stage_renders_three_files():
    ...
    assert all("dbos-ansible-local" in t for t in targets)
